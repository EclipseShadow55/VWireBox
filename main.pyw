...
if __name__ == "__main__":
    # Create new log file
    os.environ["LOG_FILE"] = log_file = f"{datetime.now().strftime('%d/%m/%Y_%H:%M:%S.%f')}.txt"
    with open(log_file, "x") as f:
        f.write("")
    # Find and initialize plugins
    plugins = []
    for direct in misc.get_subdirs("Plugins"):
        if os.path.isfile(f"Plugins/{direct}/main.py") and updater.exists("EShadow55", "Twitch-Connector", f"Plugins/{direct}/main.py"):
            updater.update_file("EShadow55", "Twitch-Connector", f"Plugins/{direct}/main.py")
            plugins.append({
                "name": direct,
